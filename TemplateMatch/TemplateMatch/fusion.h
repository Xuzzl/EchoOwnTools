...
#define INVALID 63 // > 8 && < 128
    ...
    inline int cvTypeSize(int type)
    {
        ...
        case CV_32F:
            return 4;
        default:
            qInfo() << cv::Error::StsBadArg << ("  Invalid type");
            return -1;
        }
        return 1;
    }
    ...
    std::vector<std::shared_ptr<T>> deep_copy_shared_ptr_vec(std::vector<std::shared_ptr<T>>& ori)
    {
        ...
        for (auto& ori_ptr : ori)
        {
            res.push_back(ori_ptr->clone());
        }
        return res;
    }
    ...
    {
    public:
        FilterNode(std::string op_name_temp, int input_type_temp, int input_num_temp, int output_type_temp, int output_num_temp, int op_row_temp, int op_col_temp) :
            op_name(op_name_temp), input_type(input_type_temp), input_num(input_num_temp), output_type(output_type_temp), output_num(output_num_temp), op_row(op_row_temp), op_col(op_col_temp) {}
        ...
        void update_simple(int start_r, int start_c, int end_r, int end_c) override {
            for (int r = start_r; r < end_r; r++)
            {
                ...
                for (; c < end_c; c++, parent_buf_ptr++, buf_ptr++)
                {
                    ...
                    {
                        local_sum += (int32_t(*(parent_buf_ptr + i)) + int32_t(*(parent_buf_ptr - i))) * gauss_knl_uint32[gauss_knl_idx];
                    }
                    *buf_ptr = local_sum;
                }
            }
        ...
        void update_simd(int start_r, int start_c, int end_r, int end_c) override {
            ...
            for (int r = start_r; r < end_r; r++)
            {
                ...
                for (; c < end_c; c++, buf_ptr++)
                {
                    ...
                    for (int i = 1; i <= gauss_size / 2; i++, gauss_knl_idx++)
                    {
                        local_sum += gauss_knl_uint32[gauss_knl_idx] * (*(parent_buf_center[i]) + *(parent_buf_center[-i]));
                    }
                    *buf_ptr = int16_t(local_sum >> (2 * gauss_quant_bit));
    ...
    public:
        ...
        //            }
            ...
            {
                qInfo() << ("  TM_fusion update_simple  catch other error!!!");
            }
        }
    ...
    public:
        ...
        //    fusion_buffers_(fusion_buffers), tileRows_(tileRows), tileCols_(tileCols) {}
        ProcessManager(std::vector<std::vector<char>>& fusion_buffers, int tileRows, int tileCols) : fusion_buffers_(fusion_buffers), tileRows_(tileRows), tileCols_(tileCols) {}
        ...
        std::vector<std::shared_ptr<FilterNode>>& get_nodes()
        {
            return nodes_;
        }

        // 为了避免在应用和销毁缓冲区时浪费时间，缓冲区将是我们
        // 在所有过程中使用的最大区块，我们希望清除它们以重新开始
        void clear_buffers()
        {
        ...
        void arrange(int outRows, int outCols)
        {
            ...
            for (auto& node : nodes_)
            {
                ...
                if (footprint > maxMemoFootprint_)
                {
                    maxMemoFootprint_ = footprint;
                }
            }
            ...
            //if (fusion_buffers_.size() < 1) { fusion_buffers_.resize(1); }
        ...
        {
            if (in_v.empty() || out_v.empty() || in_v[0].empty() || out_v[0].empty())
            {
                qInfo() << ("empty input or output.");
                return;
            }
            if (!(nodes_[0]->input_num == in_v.size() && nodes_[0]->input_type == in_v[0].type())
                || !(nodes_.back()->output_num == out_v.size() && nodes_.back()->output_type == out_v[0].type()))
            {
                qInfo() << ("first or last node is not compatible.");
                return;
            }

            //#pragma region CV并行方式
            //        std::mutex _mutex;
            //        cv::parallel_for_(cv::Range(0, num_threads_), [&](const cv::Range& _range) {
            //            //auto _range = cv::Range(0, num_threads_);
            //            for (auto tid = _range.start; tid < _range.end; ++tid)
            //            {
            ...
            //
            //                cv::parallel_for_(cv::Range(0, update_rois_.size()), [&](const cv::Range& _range2) {
            //                    //auto _range2 = cv::Range(0, update_rois_.size());
            //                    for (auto roi_iter = _range2.start; roi_iter < _range2.end; ++roi_iter)
            //                    {
            ...
            //                                cv::Mat buffer_header(cur_roi.height + nodes_private[0]->padded_row * 2, cur_roi.width + nodes_private[0]->padded_col * 2, in.type(), buffer_cur);
            //
            ...
            //                                {
            //                                    // data is not owned by mat if initialize it this way
            //                                    cv::Mat buffer_header(cur_roi.height + cur_node->padded_row * 2, cur_roi.width + cur_node->padded_col * 2, cur_node->input_type, buffer_cur);
            //                                    
            //                                    int tile_size = buffer_header.rows * buffer_header.cols;
            ...
            //                            if (nodes_private.back()->op_name != "linearizeTxT")
            //                            {
            //                                // linearize is a very special operations
            ...
            //
            //                        // update one by one
            //                        for (int i = 0; i < nodes_private.size(); i++)
            //                        {
            //                            nodes_private[i]->update();
            //                        }
            //                    }
            //                    });
            //
...
#pragma endregion
    ...
    bool check_if_nodes_valid()
    {
        ...
        for (int i = 0; i < nodes_.size() - 1; i++)
        {
            ...
            if (cur_node->output_num != next_node->input_num || cur_node->output_type != next_node->input_type)
            {
                qInfo() << "node " << i << "is not compatible with node" << i + 1;
                is_valid = false;
            }
        }
        return is_valid;
